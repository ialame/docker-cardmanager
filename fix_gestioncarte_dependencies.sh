...
# 🖼️ Correction Dépendances GestionCarte
# Résout le problème des versions manquantes dans le conteneur GestionCarte

echo "🖼️ Correction Dépendances GestionCarte"
echo "======================================"
...
echo -e "${BLUE}🎯 Problème identifié : GestionCarte n'hérite pas du dependencyManagement${NC}"
echo -e "${YELLOW}📋 Solution : Dockerfile GestionCarte avec dependencyManagement complet${NC}"
echo ""

# 1. Sauvegarder le Dockerfile actuel
echo -e "${BLUE}💾 Sauvegarde du Dockerfile GestionCarte...${NC}"
[ -f "docker/gestioncarte/Dockerfile" ] && cp docker/gestioncarte/Dockerfile docker/gestioncarte/Dockerfile.before-deps-fix
echo -e "${GREEN}✅ Dockerfile sauvegardé${NC}"

# 2. Correction du Dockerfile GestionCarte avec le même POM que Painter
echo -e "${BLUE}🖼️ Création du Dockerfile GestionCarte avec dependencyManagement...${NC}"
mkdir -p docker/gestioncarte
cat > docker/gestioncarte/Dockerfile << 'EOF'
# 🖼️ Dockerfile GestionCarte - Version avec DependencyManagement Complet
...
# Créer le MÊME POM parent complet que dans Painter
...
# Scripts de clone et build avec retry
...
# Script Maven avec retry intelligent
...
# Construire Painter avec retry et dependencyManagement
...
RUN echo "🔍 Vérification Painter installé:" && \
    ls -la ~/.m2/repository/com/pcagrade/painter/ && \
    echo "✅ Painter disponible pour GestionCarte"

# Construire GestionCarte avec retry
WORKDIR /usr/src/app/gestioncarte
RUN /usr/local/bin/maven-build-retry.sh "GestionCarte" "mvn clean package -DskipTests -Dmaven.test.skip=true -B"
...
echo -e "${GREEN}✅ Dockerfile GestionCarte corrigé avec dependencyManagement complet${NC}"

# 3. Script de build pour uniquement GestionCarte
echo -e "${BLUE}🚀 Création du script de build GestionCarte...${NC}"
cat > build-gestioncarte-only.sh << 'EOF'
#!/bin/bash
...
chmod +x build-gestioncarte-only.sh
echo -e "${GREEN}✅ Script de build GestionCarte créé${NC}"

# Résumé
echo ""
echo -e "${GREEN}🎉 CORRECTION GESTIONCARTE TERMINÉE !${NC}"
echo -e "${BLUE}════════════════════════════════════${NC}"
echo ""
echo -e "${YELLOW}📋 Problème résolu :${NC}"
echo "   ❌ AVANT : GestionCarte n'avait pas les versions Mason"
echo "   ✅ APRÈS : Même dependencyManagement que Painter"
echo ""
echo -e "${YELLOW}🔧 Améliorations :${NC}"
echo "   ✅ Dockerfile GestionCarte identique à Painter"
echo "   ✅ DependencyManagement complet avec toutes les versions"
echo "   ✅ Mason installé AVANT Painter"
echo "   ✅ Painter installé AVANT GestionCarte"
echo "   ✅ Build avec retry pour chaque projet"
echo "   ✅ Diagnostic à chaque étape"
echo ""
echo -e "${YELLOW}🚀 Pour continuer :${NC}"
echo "   ./build-gestioncarte-only.sh"
echo ""
echo -e "${YELLOW}💡 Logique de build :${NC}"
echo "   1. Mason construit et installé (repository Maven)"
echo "   2. Painter construit avec dépendances Mason (repository Maven)"
echo "   3. GestionCarte construit avec dépendances Mason + Painter"
echo ""
echo -e "${GREEN}Les dépendances GestionCarte devraient maintenant être résolues ! 🎯${NC}"